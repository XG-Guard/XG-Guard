...
async def defense_communication(ag:AgentGraphWithDefense, gnn: MyGAT, qa_data, adj_m: np.ndarray,  num_dialogue_turns, topk): 
    communication_data = []
    response_embeddings = []
    initial_responses = await ag.afirst_generate(qa_data)
...
async def main(): 
    ...
    # state_dict = torch.load(args.gnn_checkpoint_path, map_location=torch.device('cpu'), weights_only=True)
    ...
    for d in tqdm(dataset): 
        ...
        try:
            if args.get_no_defense:
                agnd = AgentGraph(adj_m, system_prompts, attacker_idxes, model_type=args.model_type)
            ...
            communication_data_defense, original_predicts = await defense_communication(agwd, gnn, qa_data, adj_m, num_dialogue_turns, args.topk)
