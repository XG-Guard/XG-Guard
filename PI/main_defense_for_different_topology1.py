...
def embeddings2graph_last(embeddings, adj_matrix):
...
async def defense_communication(ag:AgentGraphWithDefense, gnn: MyGAE, qa_data, adj_m: np.ndarray,  num_dialogue_turns, defend_type, topk, attacker_idxes): 
    communication_data = []
    response_embeddings = []
    initial_responses = await ag.afirst_generate(qa_data)
    ...
    for _ in range(num_dialogue_turns): 
        ...
        # PREM-GAD
            ...
            # x_token = torch.stack([t.mean(dim=0) for t in embeddings_tokenlevel])
            # x_token_ori = embeddings_tokenlevel
            # emb_sentance, emb_token, context_sentance, context_token = gnn.forward(x_sentance, x_token, x_token_ori,
            #                                                                        edge_index)
            # score_sentance = gnn.inference(emb_sentance, context_sentance)
            # score_token, score_finegrain = gnn.inference_token(emb_token, context_token)
            # anomaly_scores_new = get_score_overall(score_sentance, score_token)
            # anomaly_scores, _ = torch.stack([anomaly_scores, anomaly_scores_new]).max(dim=0)
            # value, predicts = torch.topk(anomaly_scores, topk)
...
def parse_arguments():
    ...
    # parser.add_argument("--gnn_checkpoint_path", type=str, default="G:/AgentGAD/BlindGuard/MA/ckpt/PI-CSQA_seed3701_alpha0.0001_lr1e-05.pkl")
    parser.add_argument("--gnn_checkpoint_path", type=str, default="G:/AgentGAD/BlindGuard/MA/ckpt/PI-MMLU_seed3701_alpha0.0001_lr0.0001.pkl")
...
async def main(): 
    ...
    for d in tqdm(dataset): 
        ...
        communication_data_defense, original_predicts = await defense_communication(agwd, gnn, qa_data, adj_m, num_dialogue_turns, args.defend_type, args.topk, attacker_idxes)
